...
void test_frame_state1(void)
{
    ...
    struct nmm_base_table const* baset =
        nmm_base_table_create(base, logf(0.25), logf(0.25), logf(0.25), logf(0.25));
...
void test_frame_state2(void)
{
    ...
    struct imm_abc const*        abc = nmm_base_abc_super(base);
    struct nmm_base_table const* baset =
        nmm_base_table_create(base, logf(0.1), logf(0.2), logf(0.3), logf(0.4));
...
void test_frame_state3(void)
{
    ...
    struct nmm_codon*       codon = nmm_codon_create(base);
    nmm_codon_set_triplet(codon, NMM_TRIPLET('A', 'T', 'G'));
    nmm_codon_lprob_set(codonp, codon, logf(0.8) - logf(1.3));
    nmm_codon_set_triplet(codon, NMM_TRIPLET('A', 'T', 'T'));
    nmm_codon_lprob_set(codonp, codon, logf(0.1) - logf(1.3));
    nmm_codon_set_triplet(codon, NMM_TRIPLET('G', 'T', 'C'));
    nmm_codon_lprob_set(codonp, codon, logf(0.4) - logf(1.3));
    struct nmm_codon_table const* codont = nmm_codon_table_create(codonp);
    nmm_codon_lprob_destroy(codonp);
...
void test_frame_state_lposterior(void)
{
    ...
    while ((codon_item = cartes_next(codon_iter)) != NULL) {
        ...
        nmm_codon_lprob_set(codonp, codon, logf(0.001));
    }
    ...
    nmm_codon_set_triplet(codon, NMM_TRIPLET('G', 'T', 'C'));
    nmm_codon_lprob_set(codonp, codon, logf(0.4));
    ...
    while ((codon_item = cartes_next(codon_iter)) != NULL) {
        ...
        for (unsigned times = 1; times < 6; ++times) {
            ...
            while ((seq = cartes_next(seq_iter)) != NULL) {
                ...
                cass_cond(tmp != NULL);
                float lprob = nmm_frame_state_lposterior(state, codon, tmp);
                imm_seq_destroy(tmp);
                lprob -= nmm_codon_table_lprob(codont, codon);
