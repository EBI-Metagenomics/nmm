...
void test_codont_works(void)
{
    ...
    cass_cond(nmm_codonp_set(lprobs, &NMM_CODON('A', 'T', 'G'), log(0.8)) == 0);
    cass_cond(nmm_codonp_set(lprobs, &NMM_CODON('A', 'T', 'T'), log(0.1)) == 0);
    cass_cond(nmm_codonp_set(lprobs, &NMM_CODON('A', 'H', 'T'), log(0.1)) == 1);
