...
void test_codont_marginal(void)
{
    ...
    cass_cond(nmm_codon_set(codon, NMM_TRIPLET('A', 'T', 'T')) == 0);
    cass_close(nmm_codont_lprob(codont, codon), log(0.1));
    cass_cond(nmm_codon_set(codon, NMM_TRIPLET('A', 'T', 'X')) == 0);
    cass_close(nmm_codont_lprob(codont, codon), log(0.9));
    cass_cond(nmm_codon_set(codon, NMM_TRIPLET('A', 'X', 'X')) == 0);
    cass_close(nmm_codont_lprob(codont, codon), log(0.9));
    cass_cond(nmm_codon_set(codon, NMM_TRIPLET('X', 'X', 'X')) == 0);
    cass_close(nmm_codont_lprob(codont, codon), log(0.9));
    cass_cond(nmm_codon_set(codon, NMM_TRIPLET('X', 'T', 'X')) == 0);
    cass_close(nmm_codont_lprob(codont, codon), log(0.9));
    cass_cond(nmm_codon_set(codon, NMM_TRIPLET('X', 'X', 'G')) == 0);
    cass_close(nmm_codont_lprob(codont, codon), log(0.8));
    cass_cond(nmm_codon_set(codon, NMM_TRIPLET('X', 'X', 'T')) == 0);
    cass_close(nmm_codont_lprob(codont, codon), log(0.1));
    nmm_codon_destroy(codon);
