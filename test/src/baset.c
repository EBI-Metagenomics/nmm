...
void test_baset(void)
{
    ...
    struct nmm_baset const* baset = nmm_baset_create(base, log(8), log(1), zero, log(3));

    cass_close(nmm_baset_lprob(baset, 'A'), log(8));
    cass_close(nmm_baset_lprob(baset, 'C'), log(1));
    cass_cond(imm_lprob_is_zero(nmm_baset_lprob(baset, 'G')));
    cass_close(nmm_baset_lprob(baset, 'T'), log(3));
