...
struct nmm_base_lprob const* nmm_base_lprob_create(struct nmm_base_abc const* abc, imm_float const a, imm_float const b,
...
struct nmm_base_lprob const* base_lprob_read(FILE* stream, struct nmm_base_abc const* abc)
{
    ...
    if (fread(basep->lprobs, sizeof(*basep->lprobs), NMM_BASE_ABC_SIZE, stream) < NMM_BASE_ABC_SIZE) {
...
{
    if (fwrite(basep->lprobs, sizeof(*basep->lprobs), NMM_BASE_ABC_SIZE, stream) < NMM_BASE_ABC_SIZE) {
