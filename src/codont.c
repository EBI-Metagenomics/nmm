                                     ...
                                     struct nmm_codon_lprob const* lprobs, int lprobs_length)
{
    ...
    codont->abc = abc;
    codont->lprob = nmm_codon_lmarg_create(abc, lprobs, lprobs_length);

    if (!codont->lprob) {
        nmm_codon_lmarg_destroy(codont->lprob);
        free(codont);
        return NULL;
    }
...
{
    return codon_lmarg_get(codont->lprob, codon);
}
