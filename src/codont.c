...
static void set_marginal_lprobs(struct nmm_codont* codont, struct imm_abc const* abc);
...
struct nmm_codont const* nmm_codont_create(struct nmm_codonp const* codonp)
{
    ...
    set_marginal_lprobs(codont, abc);
...
static int set_nonmarginal_lprobs(struct nmm_codont* codont, struct nmm_codonp const* codonp)
{
    struct imm_abc const* abc = nmm_base_get_abc(nmm_codonp_get_base(codonp));
    ...
    for (int a = 0; a < shape[0]; ++a) {
        for (int b = 0; b < shape[1]; ++b) {
            for (int c = 0; c < shape[2]; ++c) {
                ...
                lprob = logaddexp(lprob, codon_lmarg_get(codont, &t));
            }
        }
    }
