...
static inline float                   base_lprob(struct nmm_frame_state const* state, char id);
...
static float        joint_seq_len1(struct nmm_frame_state const* state, struct imm_seq const* seq);
static float        joint_seq_len2(struct nmm_frame_state const* state, struct imm_seq const* seq);
static float        joint_seq_len3(struct nmm_frame_state const* state, struct imm_seq const* seq);
static float        joint_seq_len4(struct nmm_frame_state const* state, struct imm_seq const* seq);
static float        joint_seq_len5(struct nmm_frame_state const* state, struct imm_seq const* seq);
...
static float lprob_frag_given_codon1(struct nmm_frame_state const* state, struct imm_seq const* seq,
                                     struct nmm_codon const* ccode);
static float lprob_frag_given_codon2(struct nmm_frame_state const* state, struct imm_seq const* seq,
                                     struct nmm_codon const* ccode);
static float lprob_frag_given_codon3(struct nmm_frame_state const* state, struct imm_seq const* seq,
                                     struct nmm_codon const* ccode);
static float lprob_frag_given_codon4(struct nmm_frame_state const* state, struct imm_seq const* seq,
                                     struct nmm_codon const* ccode);
static float lprob_frag_given_codon5(struct nmm_frame_state const* state, struct imm_seq const* seq,
                                     struct nmm_codon const* ccode);
                                                     ...
                                                     float const                   epsilon)
{
    ...
    state->leps = logf(epsilon);
    state->l1eps = logf(1 - epsilon);
                             ...
                             struct nmm_codon* codon)
{
    ...
    float             max_lprob = state->zero_lprob;
    ...
    for (unsigned i0 = 0; i0 < n; ++i0) {
        for (unsigned i1 = 0; i1 < n; ++i1) {
            for (unsigned i2 = 0; i2 < n; ++i2) {
                ...
                float lprob = nmm_frame_state_lposterior(state, tmp, seq);
...
float nmm_frame_state_epsilon(struct nmm_frame_state const* state) { return state->epsilon; }

float nmm_frame_state_lposterior(struct nmm_frame_state const* state, struct nmm_codon const* codon,
                                 struct imm_seq const* seq)
{
    float    lprob = state->zero_lprob;
...
struct imm_state const* nmm_frame_state_read(FILE* stream, struct nmm_model const* model)
{
    ...
    frame_state->leps = logf(frame_state->epsilon);
    frame_state->l1eps = logf(1 - frame_state->epsilon);
...
static float joint_seq_len1(struct nmm_frame_state const* state, struct imm_seq const* seq)
{
    ...
    float const c = 2 * state->leps + 2 * state->l1eps;

    char const  nucl = imm_seq_string(seq)[0];
    float const e0 = nmm_codon_table_lprob(state->codont, codon_set(&codon, nucl, _, _));
    float const e1 = nmm_codon_table_lprob(state->codont, codon_set(&codon, _, nucl, _));
    float const e2 = nmm_codon_table_lprob(state->codont, codon_set(&codon, _, _, nucl));

    return c + logaddexp3(e0, e1, e2) - logf(3);
}
...
#define C(a, b, c)                                                                                 \
    ...
    float const b_lp0 = base_lprob(state, str[0]);
    float const b_lp1 = base_lprob(state, str[1]);

    float const c0 = logf(2) + state->leps + state->l1eps * 3 - logf(3);
    float const v0 = c0 + logaddexp3(C(_, 0, 1), C(0, _, 1), C(0, 1, _));

    float const c1 = 3 * state->leps + state->l1eps - logf(3);
    float const v1 = c1 + logaddexp3(C(0, _, _), C(_, 0, _), C(_, _, 0)) + b_lp1;
    float const v2 = c1 + logaddexp3(C(1, _, _), C(_, 1, _), C(_, _, 1)) + b_lp0;
...
#define C(a, b, c)                                                                                 \
    ...
    float const B[] = {base_lprob(state, str[0]), base_lprob(state, str[1]),
                       base_lprob(state, str[2])};

    float const v0 = 4 * state->l1eps + C(0, 1, 2);

    float const c1 = logf(4) + 2 * state->leps + 2 * state->l1eps - logf(9);
    float const v1[] = {logaddexp3(C(_, 1, 2), C(1, _, 2), C(1, 2, _)) + B[0],
                        logaddexp3(C(_, 0, 2), C(0, _, 2), C(0, 2, _)) + B[1],
                        logaddexp3(C(_, 0, 1), C(0, _, 1), C(0, 1, _)) + B[2]};

    float const c2 = 4 * state->leps - logf(9);
    float const v2[] = {logaddexp3(C(2, _, _), C(_, 2, _), C(_, _, 2)) + B[0] + B[1],
...
#define C(a, b, c)                                                                                 \
    ...
    float const c0 = state->leps + state->l1eps * 3 - logf(2);
    float const v0[] = {C(1, 2, 3) + B[0], C(0, 2, 3) + B[1], C(0, 1, 3) + B[2], C(0, 1, 2) + B[3]};

    float const c1 = 3 * state->leps + state->l1eps - logf(9);
    float const v1[] = {
...
#define C(a, b, c) codon_set(&codon, str[a], str[b], str[c])
    ...
    float const b_lp0 = base_lprob(state, str[0]);
    float const b_lp1 = base_lprob(state, str[1]);
    float const b_lp2 = base_lprob(state, str[2]);
    float const b_lp3 = base_lprob(state, str[3]);
    float const b_lp4 = base_lprob(state, str[4]);
    ...
    return 2 * state->leps + 2 * state->l1eps - logf(10) + imm_lprob_sum(v, 5);
#undef C
#undef LPROB
}
...
{
    float const loge = state->leps;
    float const log1e = state->l1eps;
    ...
    float const c = 2 * loge + 2 * log1e;

    return c + logf((x1 == z1) + (x2 == z1) + (x3 == z1)) - logf(3);
}
                                     ...
                                     struct nmm_codon const* codon)
{
    ...
    float const lprob_z1 = base_lprob(state, z1);
    float const lprob_z2 = base_lprob(state, z2);

    float const c1 = logf(2) + loge + log1e * 3 - logf(3);
    float const v0 =
        c1 + logf((x2 == z1) * (x3 == z2) + (x1 == z1) * (x3 == z2) + (x1 == z1) * (x2 == z2));

    float const c2 = 3 * loge + log1e - logf(3);

    float const v1 = c2 + logf((x1 == z1) + (x2 == z1) + (x3 == z1)) + lprob_z2;
    float const v2 = c2 + logf((x1 == z2) + (x2 == z2) + (x3 == z2)) + lprob_z1;
                                     ...
                                     struct nmm_codon const* codon)
{
    ...
    float const lprob_z1 = base_lprob(state, z1);
    float const lprob_z2 = base_lprob(state, z2);
    float const lprob_z3 = base_lprob(state, z3);

    float const v0 = 4 * log1e + logf((x1 == z1) * (x2 == z2) * (x3 == z3));

    float const c1 = logf(4) + 2 * loge + 2 * log1e - logf(9);

    float const v1 =
        c1 + logf((x2 == z2) * (x3 == z3) + (x1 == z2) * (x3 == z3) + (x1 == z2) * (x2 == z3)) +
        lprob_z1;

    float const v2 =
        c1 + logf((x2 == z1) * (x3 == z3) + (x1 == z1) * (x3 == z3) + (x1 == z1) * (x2 == z3)) +
        lprob_z2;
    ...
    float const c2 = 4 * loge - logf(9);

    float const v4 = c2 + logf((x1 == z3) + (x2 == z3) + (x3 == z3)) + lprob_z1 + lprob_z2;
    float const v5 = c2 + logf((x1 == z2) + (x2 == z2) + (x3 == z2)) + lprob_z1 + lprob_z3;
    float const v6 = c2 + logf((x1 == z1) + (x2 == z1) + (x3 == z1)) + lprob_z2 + lprob_z3;

    float v[] = {v0, v1, v2, v3, v4, v5, v6};
                                     ...
                                     struct nmm_codon const* codon)
{
    ...
    float const lprob_z2 = base_lprob(state, z2);
    float const lprob_z3 = base_lprob(state, z3);
    float const lprob_z4 = base_lprob(state, z4);

    float const v0[] = {logf((x1 == z2) * (x2 == z3) * (x3 == z4)) + lprob_z1,
                        logf((x1 == z1) * (x2 == z3) * (x3 == z4)) + lprob_z2,
                        logf((x1 == z1) * (x2 == z2) * (x3 == z4)) + lprob_z3,
                        logf((x1 == z1) * (x2 == z2) * (x3 == z3)) + lprob_z4};

    float const v1[] = {
        logf((x2 == z3) * (x3 == z4)) + lprob_z1 + lprob_z2,
        logf((x2 == z2) * (x3 == z4)) + lprob_z1 + lprob_z3,
        logf((x2 == z2) * (x3 == z3)) + lprob_z1 + lprob_z4,
        logf((x2 == z1) * (x3 == z4)) + lprob_z2 + lprob_z3,
        logf((x2 == z1) * (x3 == z3)) + lprob_z2 + lprob_z4,
        logf((x2 == z1) * (x3 == z2)) + lprob_z3 + lprob_z4,
        logf((x1 == z3) * (x3 == z4)) + lprob_z1 + lprob_z2,
        logf((x1 == z2) * (x3 == z4)) + lprob_z1 + lprob_z3,
        logf((x1 == z2) * (x3 == z3)) + lprob_z1 + lprob_z4,
        logf((x1 == z1) * (x3 == z4)) + lprob_z2 + lprob_z3,
        logf((x1 == z1) * (x3 == z3)) + lprob_z2 + lprob_z4,
        logf((x1 == z1) * (x3 == z2)) + lprob_z3 + lprob_z4,
        logf((x1 == z3) * (x2 == z4)) + lprob_z1 + lprob_z2,
        logf((x1 == z2) * (x2 == z4)) + lprob_z1 + lprob_z3,
        logf((x1 == z2) * (x2 == z3)) + lprob_z1 + lprob_z4,
        logf((x1 == z1) * (x2 == z4)) + lprob_z2 + lprob_z3,
        logf((x1 == z1) * (x2 == z3)) + lprob_z2 + lprob_z4,
        logf((x1 == z1) * (x2 == z2)) + lprob_z3 + lprob_z4,
    };

    return imm_lprob_add(loge + log1e * 3 - logf(2) + imm_lprob_sum(v0, NMM_ARRAY_SIZE(v0)),
                         3 * loge + log1e - logf(9) + imm_lprob_sum(v1, NMM_ARRAY_SIZE(v1)));
}
                                     ...
                                     struct nmm_codon const* codon)
{
    ...
    float const lprob_z3 = base_lprob(state, z3);
    float const lprob_z4 = base_lprob(state, z4);
    float const lprob_z5 = base_lprob(state, z5);

    float const v[] = {
        lprob_z1 + lprob_z2 + logf((x1 == z3) * (x2 == z4) * (x3 == z5)),
        lprob_z1 + lprob_z3 + logf((x1 == z2) * (x2 == z4) * (x3 == z5)),
        lprob_z1 + lprob_z4 + logf((x1 == z2) * (x2 == z3) * (x3 == z5)),
        lprob_z1 + lprob_z5 + logf((x1 == z2) * (x2 == z3) * (x3 == z4)),
        lprob_z2 + lprob_z3 + logf((x1 == z1) * (x2 == z4) * (x3 == z5)),
        lprob_z2 + lprob_z4 + logf((x1 == z1) * (x2 == z3) * (x3 == z5)),
        lprob_z2 + lprob_z5 + logf((x1 == z1) * (x2 == z3) * (x3 == z4)),
        lprob_z3 + lprob_z4 + logf((x1 == z1) * (x2 == z2) * (x3 == z5)),
        lprob_z3 + lprob_z5 + logf((x1 == z1) * (x2 == z2) * (x3 == z4)),
        lprob_z4 + lprob_z5 + logf((x1 == z1) * (x2 == z2) * (x3 == z3)),
    };

    return 2 * loge + 2 * log1e - logf(10) + imm_lprob_sum(v, NMM_ARRAY_SIZE(v));
}
